...
export default function TasksScreen({ navigation }: any) {
    ...
    const tabBgColor = colorScheme === 'dark' ? '#27272a' : '#f4f4f5';
    const tabActiveBgColor = colorScheme === 'dark' ? '#3f3f46' : '#ffffff';
    ...
    return (
        <ScreenWrapper>
            ...
            {/* Tabs */}
            <View style={{ backgroundColor: tabBgColor }} className="flex-row p-1 rounded-xl mb-6">
                <TouchableOpacity
                    onPress={() => setTab('active')}
                    style={{ backgroundColor: tab === 'active' ? tabActiveBgColor : 'transparent' }}
                    className={`flex-1 py-2 items-center rounded-lg ${tab === 'active' ? 'shadow-sm' : ''}`}
                >
                    <Text className={`font-bold ${tab === 'active' ? 'text-zinc-900' : 'text-zinc-500'}`}>To Do</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    onPress={() => setTab('done')}
                    style={{ backgroundColor: tab === 'done' ? tabActiveBgColor : 'transparent' }}
                    className={`flex-1 py-2 items-center rounded-lg ${tab === 'done' ? 'shadow-sm' : ''}`}
                >
                    <Text className={`font-bold ${tab === 'done' ? 'text-zinc-900' : 'text-zinc-500'}`}>Completed</Text>
                </TouchableOpacity>
            </View>
            ...
            {/* Add Task Modal */}
            <Modal visible={isAddTaskModalOpen} animationType="slide" presentationStyle="pageSheet">
                <AddTaskModalContent onClose={() => setIsAddTaskModalOpen(false)} />
            </Modal>
        </ScreenWrapper>
    );
}
...
const TaskItem = ({ title, subject, due, isUrgent, isDone }: any) => {
    const { colorScheme } = useColorScheme();
    const dueBgColor = isUrgent 
        ? (colorScheme === 'dark' ? '#7c2d12' : '#fee2e2')
        : (colorScheme === 'dark' ? '#27272a' : '#f4f4f5');
    const dueTextColor = isUrgent ? '#dc2626' : '#71717a';
    ...
    return (
        <View className="flex-row items-center bg-white dark:bg-zinc-900 p-4 rounded-2xl mb-3 border border-zinc-100 dark:border-zinc-800">
            ...
            <View style={{ backgroundColor: dueBgColor }} className="px-2 py-1 rounded-md">
                <Text style={{ color: dueTextColor }} className="text-xs font-bold">{due}</Text>
            </View>
        </View>
    );
};